...
static int DisplayVersion(char **vp)
{
	...
	if (vp)
		*vp = vers_info_str;
		...
		kkprintf("\nCTC Driver <Vers. %d>\n%s",
			 CTC_DRVR_CURRENT_VERSION,
			 vers_info_str);

	return CTC_DRVR_CURRENT_VERSION;
}
...
#define INST_OK()						\
do {								\
  kkprintf("Ctc: Driver (LUN %d) ", info->mlun);	\
...
char* Ctc_install(void *infofile)
{
  ...
  kkprintf("Ctc: Driver (LUN %d) installation begins...\n", info->mlun);
  ...
  */
	...
	if (info->chrindex == -1)
		kkprintf("Ctc: %sModule presence will NOT be"
			 " checked!\n", WARNING_MSG);
	else
		kkprintf("Ctc: Module presence will be checked during"
	...
	kkprintf("Ctc: Mapping device address space - ");
